...
class BrasileiraoCornerTests(unittest.TestCase):
    ...
    def test_match_with_two_placeholder_rows_is_skipped(self) -> None:
        placeholder = ",0,0,0,0,0,0,0,0\n"
        self.assertEqual(self._load(
            MATCH_HEADER + "1,01/04/2025,A,B\n",
            STAT_HEADER + "1,A" + placeholder + "1,B" + placeholder,
        ), [])

    def test_one_placeholder_and_one_real_row_is_an_error(self) -> None:
        with self.assertRaisesRegex(BrasileiraoError, "placeholder.*only one team"):
            self._load(
                MATCH_HEADER + "1,01/04/2025,A,B\n",
                STAT_HEADER
                + "1,A,0,0,0,0,0,0,0,0\n"
                + "1,B,4,8,3,400,12,2,0,1\n",
            )

    def test_legitimate_zero_corners_with_activity_is_accepted(self) -> None:
        observations = self._load(
            MATCH_HEADER + "1,01/04/2025,A,B\n",
            STAT_HEADER
            + "1,A,0,8,3,400,12,2,0,1\n"
            + "1,B,4,9,5,390,10,1,0,2\n",
        )
        self.assertEqual(
            [(item.team, item.corners_for) for item in observations],
            [("A", 0), ("B", 4)],
        )
