...
_STAT_FIELDS = ("partida_id", "clube", *_ACTIVITY_FIELDS)
...
def _is_placeholder_statistics_row(row: dict[str, str | None]) -> bool:
    """Return whether every upstream match-activity value is zero or blank."""
...
def _read_statistics(
    ...
    for row_number, row in enumerate(reader, start=2):
        try:
            ...
            match_statistics[club] = _TeamStatistics(
                corners, _is_placeholder_statistics_row(row),
            )
        except (KeyError, TypeError, ValueError) as error:
            raise BrasileiraoError(
