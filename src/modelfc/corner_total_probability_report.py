"""Report historical match-total corner probability accuracy."""
...
def main() -> None:
    ...
    source.add_argument("--history", type=Path, nargs="+")
    ...
    parser.add_argument("--models", choices=CORNER_FIXTURE_MODELS, nargs="+",
                        default=list(CORNER_FIXTURE_MODELS))
    ...
    try:
        ...
        if args.data_config:
            if (not args.competition or args.provider != "football-data"
                    or args.country or args.league):
                raise ValueError(
                    "--data-config requires --competition and no other provider selectors"
                )
        ...
        reports = [
            evaluate_match_total_probabilities(
                observations, model=model, lines=args.lines,
    ...
    print(f"History: {args.data_config or ', '.join(str(path) for path in args.history)}")
    if args.competition:
    ...
    for report in reports:
        print("\n" + format_match_total_probability_report(report))
