...
Some periods in the upstream source contain placeholder team-statistics rows
whose match-activity fields are all zero or blank. The provider detects these
rows structurally across corners, shots, passes, fouls, cards, and offsides—
rather than hardcoding seasons—and excludes matches with two placeholder rows.
