      ...
      {parsed && validation ? (
        <section className="panel review-panel" aria-labelledby="review-title">
          ...
          <div className="fixture-editor">
            <label><span>Competition code</span><input aria-invalid={Boolean(validation.fixtureErrors.competition || validation.competitionError)} list="competition-codes" value={parsed.fixture.competition} onChange={(event) => updateFixture({ competition: event.target.value.toLocaleUpperCase() })} /><small>{validation.fixtureErrors.competition ?? competitionLabels[parsed.fixture.competition] ?? "Use a code reported by the backend"}</small></label>
